...
for (essai in 1:10){
  set.seed(21*essai)
  dossier_enreg = paste0("results_minsup_ESBL/minsup_0_001/rep", essai, "/")
  chemin_donnees = paste0("results_minsup_ESBL/minsup_0_001/rep", essai, "/sampled_datasets/")
  minsup_BLSE = 0.001
...
# Densité minsup = 0.001 pour 2018 à 2022
dens <- sapply(
  sprintf("results_minsup_ESBL/minsup_0_001/rep%d/describ_graphs_0.95.xlsx", 1:10),
  function(f) read.xlsx(f)[c(1:5), 3]
)
dimnames(dens) <- list(year = as.character(2018:2022), rep = paste0("rep", 1:10))
dens
...
cor.test(x=stats_dens[,1], 1:5, method="pearson")
